...
int main(int argc, char** argv){
    ...
    //
    unsigned int dummy_height = 1000;
    unsigned int dummy_width = 1000;
   ...
   // Filling the params
    ...
    cl_kernel kernel = clCreateKernel(program, "brightness_contrast_SS", &err);
    ushort roi = 0;
    // Arguments Setting//
    int ctr = 0;
    err = clSetKernelArg(kernel, ctr++, sizeof(cl_mem), &d_input);
    err = clSetKernelArg(kernel, ctr++, sizeof(cl_mem), &d_output);
    err = clSetKernelArg(kernel, ctr++, sizeof(float), &alpha);
    err = clSetKernelArg(kernel, ctr++, sizeof(float), &beta);
    ...
    err = clSetKernelArg(kernel, ctr++, sizeof(unsigned int), &dummy_height);
    err = clSetKernelArg(kernel, ctr++, sizeof(unsigned int), &dummy_width);
